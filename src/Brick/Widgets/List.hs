...
handleListEvent e theList = case e of
...
handleListEventVi
  ...
  -> GenericList n t e
  -> EventM n (GenericList n t e)
handleListEventVi fallback e theList = case e of
  ...
  EvKey (KChar 'G') [] ->
    return $ listMoveTo (length $ listElements theList) theList
...
drawListElements
  ...
  -> (Int -> Bool -> e -> Widget n)
  -> Widget n
drawListElements foc l drawElem = Widget Greedy Greedy $ do
  c <- getContext
...
listRemove pos l
  ...
  = let newSel = case l ^. listSelectedL of
    ...
    in  l
          &  listSelectedL
          .~ (if null es' then Nothing else Just newSel)
          &  listElementsL
          .~ es'
...
listReplace es idx l =
  let l'     = l & listElementsL .~ es
      ...
      inBoundsOrZero i | i == splitClamp l' i = i
                       | otherwise            = 0
...
listMoveUp
  :: (Foldable t, Splittable t) => GenericList n t e -> GenericList n t e
...
listMoveByPages pages theList = do
  ...
  case v of
    ...
    Just vp -> do
      let nElems =
            round $ pages * fromIntegral (vp ^. vpSize . _2) / fromIntegral
              (theList ^. listItemHeightL)
...
-- @
listMoveBy
  :: (Foldable t, Splittable t) => Int -> GenericList n t e -> GenericList n t e
listMoveBy amt l =
...
-- @
listSelectedElement
  :: (Splittable t, Foldable t) => GenericList n t e -> Maybe (Int, e)
...
-- @
listReverse
  :: (Reversible t, Foldable t) => GenericList n t e -> GenericList n t e
listReverse l =
  l & listElementsL %~ reverse & listSelectedL %~ fmap (length l - 1 -)
...
-- /O(n)/).
listModify
  :: (Traversable t) => (e -> e) -> GenericList n t e -> GenericList n t e
...
I feel like if they were lists I just write it in patter matching but
1. I would like to keep them as vectors since the list will mutate a lot and
2. I feel like there is some way to do this with traverse that I'm missing
-}
